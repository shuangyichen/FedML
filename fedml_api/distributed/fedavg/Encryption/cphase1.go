...
//export genShareforNewUser
func genShareforNewUser(numPeers int, k int, logDegree uint64,shamirShare []uint64, reconstruction_coeff []uint64, user_id int)(resPointer uintptr){
    ...
    pi.shamirShare.SetCoefficients(unsqueezedArray(shamirShare, 2))
    evalPoint_for_new_user := uint64(numPeers)+1
    vandermonde := GenerateVandermonde(evalPoint_for_new_user, uint64(k), ringPrime)
    res := ringQP.NewPoly()
    ringQP.MulScalar(pi.shamirShare, reconstruction_coeff[user_id-1], res)
    for i := 1; i < k; i++ {
        tmp1 := ringQP.NewPoly()
        tmp2 := ringQP.NewPoly()
        ringQP.MulScalar(pi.shamirShare, reconstruction_coeff[i-1], tmp1)
        ringQP.MulScalar(tmp1, vandermonde[i], tmp2)
        ringQP.Add(tmp2, res, res)
        }
    //fmt.Println("user id", user_id)
    //fmt.Println(len(res.Coeffs))
    //fmt.Println(len(res.Coeffs[0]))
    resLength := len(res.Coeffs) * len(res.Coeffs[0])
    resList := make([]uint64,resLength)
    resList = squeezedArray(res.Coeffs)
    resPointer = uintptr(unsafe.Pointer(&resList[0]))
    return
}
