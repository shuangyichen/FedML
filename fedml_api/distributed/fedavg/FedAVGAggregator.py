...
class FedAVGAggregator(object):
    ...
    def __init__(self, train_global, test_global, all_train_data_num,
        ...
        self.ss_list = [None]*self.worker_num
        self.sample_num_dict = dict()
        self.flag_client_model_uploaded_dict = dict()
        self.flag_client_pcks_share_uploaded_dict = dict()
        self.flag_client_liveness_uploaded_dict = dict()
        self.flag_client_new_share_uploaded_dict = dict()
        for idx in range(self.worker_num):
            self.flag_client_model_uploaded_dict[idx] = False
            self.flag_client_pcks_share_uploaded_dict[idx] = False
            self.flag_client_liveness_uploaded_dict[idx] = False
            self.flag_client_new_share_uploaded_dict[idx]=False

    def add_ss(self,index, ss):
        self.ss_list[index] = ss
        self.flag_client_new_share_uploaded_dict[index] = True
    ...
    def check_whether_new_share_all_receive(self,client_chosen):
        for idx in client_chosen:
            if not self.flag_client_new_share_uploaded_dict[int(idx)-1]:
                return False
        for idx in range(self.worker_num):
            self.flag_client_new_share_uploaded_dict[idx] = False
        return True
