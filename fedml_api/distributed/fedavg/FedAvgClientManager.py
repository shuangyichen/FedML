...
class FedAVGClientManager(ClientManager):
    ...
    def register_message_receive_handlers(self):
        ...
        self.register_message_receive_handler(MyMessage.MSG_TYPE_S2C_SK, self.handle_message_sk)
        self.register_message_receive_handler(MyMessage.MSG_TYPE_S2C_SEND_RECONSTRUCTION_INFO,self.handle_message_reconstruction)
        self.register_message_receive_handler(MyMessage.MSG_TYPE_S2C_NEW_SK, self.handle_message_new_SK)

    def handle_message_new_SK(self, msg_params):
        self.SS = msg_params.get(MyMessage.MSG_ARG_NEW_SK)
        self.pk = msg_params.get(MyMessage.MSG_ARG_KEY_PUBLIC_KEY)
        print("New user official joins training")
        self.send_official_joining_to_server()

    def handle_message_reconstruction(self,msg_params):
        reconstructionCoefficients = msg_params.get(MyMessage.MSG_ARG_KEY_DECRYPTION_COEFFI)
        #print("Client ",self.get_sender_id()," get ",reconstructionCoefficients)
        shareforNew = genShareforNewUser(self.worker_num, self.k,self.log_degree,self.SS,reconstructionCoefficients,self.get_sender_id())
        shareforNew = shareforNew.tolist()
        self.send_new_share_to_server(shareforNew)

    def send_new_share_to_server(self, shareforNew):
        message = Message(MyMessage.MSG_TYPE_C2S_SEND_NEW_SHARE_TO_SERVER, self.get_sender_id(), 0)
        message.add_params(MyMessage.MSG_ARG_KEY_SS, shareforNew)
        self.send_message(message)


    def handle_message_sk(self,msg_params):
        self.SS = msg_params.get(MyMessage.MSG_ARG_KEY_SS)
        #self.SS = SS.tolist()
    ...
    def send_SS(self):
        ...
        self.send_message_ShamirShares(0,ShamirShares.tolist())
        '''
        for partyCntr in range(self.worker_num):
            if partyCntr+1 == self.get_sender_id():
                ...
                print("send to", partyCntr+1)
                self.send_message_ShamirShares(partyCntr+1,ShamirShares[partyCntr].tolist())
        '''

    def send_new_join_request(self):
        message = Message(MyMessage.MSG_TYPE_C2S_SEND_JOIN_REQUEST, self.get_sender_id(), 0)
        #message.add_params(MyMessage.MSG_ARG_KEY_LIVENESS_STATUS,self.status)
        self.send_message(message)
    ...
    def send_official_joining_to_server(self):
        message = Message(MyMessage.MSG_TYPE_C2S_OFFICIAL_JOIN, self.get_sender_id(), 0)
        self.send_message(message)
