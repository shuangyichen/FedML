...
lib.genShamirsharesforUser.argtypes = [GoSliceuint64, c_longlong, c_ulonglong]
lib.genShamirsharesforUser.restype = POINTER(c_ulonglong)
lib.genShareforNewUser.argtypes = [c_longlong, c_longlong, c_ulonglong,GoSliceuint64,GoSliceuint64,c_longlong]
lib.genShareforNewUser.restype = POINTER(c_ulonglong)
lib.genSKforNewUser.argtypes = [c_longlong,c_ulonglong,GoSliceuint64]
lib.genSKforNewUser.restype = POINTER(c_ulonglong)



def genSKforNewUser(k, logDegree, shareList):
    shareList = uint64convertToGoSlice(shareList)
    res = lib.genSKforNewUser(k,logDegree,shareList)
    return np.ctypeslib.as_array(res,shape = (16384,1))

def genShareforNewUser(numPeers, k, logDegree, shamirShare, reconstruction_coeff, user_id):
    SS = uint64convertToGoSlice_(shamirShare)
    R_coeff = uint64convertToGoSlice_(reconstruction_coeff)
    res = lib.genShareforNewUser(numPeers, k, logDegree, SS, R_coeff, user_id)
    return np.ctypeslib.as_array(res,shape = (16384,1))



def genShamirsharesforUser(shamirShares, numPeers, logDegree):
    shamirShares = uint64convertToGoSlice(shamirShares)
    shares = lib.genShamirsharesforUser(shamirShares, numPeers, logDegree)
    return np.ctypeslib.as_array(shares,shape = (16384*numPeers,1))
...
def genPCKSShare(enc_aggr_model,TPK,shamir_share,worker_num,decryptionCoefficient, inputLength, robust, logDegree, scale, numPieces):
    ...
    init = time.time()
    if not isinstance(shamir_share[0],int):
        ss = uint64convertToGoSlice(shamir_share)
    else:
        ss = uint64convertToGoSlice_(shamir_share)
    PCKSShare = lib.genPCKSShare(uint64convertToGoSlice(enc_aggr_model),uint64convertToGoSlice(TPK),ss,worker_num, decryptionCoefficient,inputLength,robust,logDegree,2.**scale)
